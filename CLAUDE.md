...
`script/apps/mac/setup.sh` installs a single `Brewfile.base` (work/personal Brewfile splits were merged into it). After installing, it offers an optional `brew bundle cleanup --force` to remove packages no longer listed in the Brewfile (default: no).

`Brewfile.lock.json` is generated by `brew bundle` and gitignored.

**Work Environment Detection** (used by `bin/dot` to pick `mac_work_installation.sh` and set `WORK_ENV`):
...
**Installation Array Pattern** (macOS):

The component list lives in `script/mac_components.sh` — the single source of truth shared by `mac_installation.sh`, `mac_work_installation.sh`, and `bin/dot`'s module-selection menu. Environment-specific components are appended inside its `WORK_ENV` conditional.
...
Linux installers still use the older inline-array + `source` pattern.
...
- **Work variant**: `script/mac_work_installation.sh` — same component list as personal (shared via `script/mac_components.sh`), but exports `WORK_ENV=1`/`DOT_ENVIRONMENT=work`
