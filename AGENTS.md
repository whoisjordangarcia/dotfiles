...
This is a focused dotfiles repository for macOS, Arch Linux, and lightweight Linux server setups. Top-level entry points are `boot.sh`, `bootstrap.sh`, and `bin/dot`. Installation logic lives in `script/`, with shared helpers in `script/common/` and component installers under paths like `script/tmux/setup.sh` or `script/apps/mac/setup.sh`. Symlinked application settings live in `configs/`, grouped by tool or platform, for example `configs/nvim/`, `configs/hypr/`, `configs/rift/`, and `configs/waybar/`. Static assets and reference docs live in `art/`, `wallpapers/`, and `docs/`.
...
- `./script/dev/check.sh`: run non-destructive health checks for shell syntax, profile module paths, and retired platform/tool references.
